...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Get task attempts using RPC
    const { data: taskAttempts, error } = await supabaseClient
      .rpc('get_user_task_attempts', { p_telegram_id: telegramUserId })

    if (error) throw error

    return new Response(
      JSON.stringify({ success: true, taskAttempts }),
